...
half* s_V_ptr = smem + (16384 * 2);           // V 从 32768 开始
            ...
            for(int k_chunk_start=seq_start;k_chunk_start<seq_end;k_chunk_start+=Block_Size){
                ...
                int global_row_start = action_row * 64; 
    int global_col_start = action_col * 64;

    #pragma unroll
    for(int i=0; i<4; ++i) {
        #pragma unroll
        for(int j=0; j<4; ++j) {
            // 计算当前 16x16 碎片在 128x128 靶场里的物理坐标
            int target_row = global_row_start + i * 16;
            int target_col = global_col_start + j * 16;
            
            // 计算目标物理指针 (Stride 设为 128)
            float* target_ptr = s_dump + target_row * 128 + target_col;
    ...
    // 倾泻完毕，直接杀死内核！不要再去算剩下的 K 块了，先把眼下的真理对齐！
    return;
